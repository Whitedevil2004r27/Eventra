    ...
    <Sidebar className="glass-card border-0 bg-gradient-to-b from-slate-900/95 to-slate-800/95 backdrop-blur-xl shadow-2xl">
      <SidebarHeader className="border-b border-slate-700/50 p-6 bg-gradient-to-r from-slate-800/50 to-slate-700/30">
        <Link to="/" className="flex items-center gap-3 hover:scale-105 transition-all duration-300 hover:rotate-1 group">
          <div className="w-12 h-12 bg-gradient-to-br from-blue-500 via-purple-500 to-orange-500 rounded-xl flex items-center justify-center shadow-xl hover:shadow-blue-500/25 transition-all duration-300 group-hover:shadow-2xl group-hover:shadow-blue-500/40 neumorphic-card p-1">
            <img 
          ...
          <div className="flex items-center gap-2">
            ...
            <Badge variant="secondary" className="text-xs bg-gradient-to-r from-blue-500/20 to-purple-500/20 text-blue-300 border border-blue-500/30 shadow-lg">
              Pro
            </Badge>
          </div>
        </Link>
                ...
                <SidebarMenuItem key={item.title}>
                  <SidebarMenuButton 
                    ...
                    className={`
                      ...
                      ${isActive(item.url) 
                        ? 'bg-gradient-to-r from-blue-500/20 to-purple-500/20 border border-blue-500/30 shadow-lg shadow-blue-500/25 text-white ring-2 ring-blue-500/50' 
                  ...
                  >
                    <Link to={item.url} className="flex items-center gap-3 p-3">
                      <item.icon className={`w-5 h-5 transition-all duration-300 group-hover:scale-110 ${isActive(item.url) ? 'text-blue-300 drop-shadow-lg' : 'text-slate-300 group-hover:text-white'}`} />
                      ...
                      {isActive(item.url) && (
                        <div className="absolute inset-0 bg-gradient-to-r from-blue-500/10 to-purple-500/10 rounded-xl animate-pulse-neon" />
                      )}
                    </Link>
                  </SidebarMenuButton>
                </SidebarMenuItem>
